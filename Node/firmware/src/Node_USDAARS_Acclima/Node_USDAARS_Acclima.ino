/*ARS Node for use with Cellular Gateway
   ...
   **Version 2020.12.07 Converts VWC from % to volume/volume (for Steve only, not PSA)
   Version 2021.01.08 Adds 10 minute interval option
   Version 2021.01.25 Remove numMissed loop
   Version 2021.03.09 Set both alarms if battV < 3.4
   Version 2021.04.14 Add one minute to listening if batt < 3.4 or numMissed == 3
   Version 2021.06.08 Use struct to save sensor metadata (addr, ID, type, meas command, depth), remove saving sensor IDs to Flash
   Version 2021.06.14 Fix menu option for changing sensor addresses
   Version 2021.06.17 Add option to print last 10 logs to print data menu option
   ...
   Version 2021.06.25 Require +/- for depths in config string
   ...
   Version 2021.08.02 Fix issue when VWC < 10% (original response gets added to VWC conversion)
...
#define EEPROM_LAT                  (200 + EEPROMSHIFT)       // store latitude
#define EEPROM_LNG                  (220 + EEPROMSHIFT)       // store longitude
...
void setActiveSDI12(char c) {   // save active sensor addresses
  //  Serial.print("sensorTot = "); Serial.println(sensorTot);
...
void listenRespond() {
  ...
  //  /// TEST ///
  //  char allData[AC_MAX_MSG_LENGTH];
  //
  //  sprintf(allData,"%s~%s~%s~%s~%lu~%d.%d", VERSION, projectID, lat, lng, serNum, battV_whole, battV_dec);
  //
  //  for(byte s = 0; s < sensorTot; s++){
  //    sprintf(allData,"~%s~%s~%s",allSensors[s].ID,allSensors[s].depth,allSensors[s].data);
  //  }
  //
  //  /// END  ///
...
//      strcpy(allSensors[rowNum].data,errString);
  ...
  if(!tdr){    // if data string not already concatenated
...
void getTimeData(byte r, char a, char data[80], byte l){
  byte rowNum = r;
  char index = a;
  byte len = l;
  char response[len];
...
void Timestamp() {      // compile timestamp
  ...
  /*
    battV_whole = battV / 1;
    int battV_d = (battV * 100);
    battV_d = battV_dec % 100;
    battV_dec = battV_d;
    ...
    Serial.println(boxTemp_whole);
    Serial.println(boxTemp_dec);
    ...
    if (mins < 10) {
      ...
      minChar[0] = (mins / 10) + 48;
      minChar[1] = (mins % 10) + 48;
    }
    ...
    if (secs < 10) {
      ...
      secChar[0] = (secs / 10) + 48;
      secChar[1] = (secs % 10) + 48;
    }

    sprintf(timestamp,"%s~%s~%s~%s~%d~%d.%d~%d.%d~%d~%d.%d~%d-%d-%d_%d:%s:%s_UTC~",VERSION,projectID,lat,lng,serNum,battV_whole,battV_dec,boxTemp_whole,boxTemp_dec,pvCurrent,pvV_whole,pvV_dec,yrs,mnths,days,hrs,minChar,secChar);
  */
  timestamp += VERSION;
  timestamp += sep;
  timestamp += projectID;
...
//      userinput = true;                        // added 01/13/2020
  ...
  switch (menuinput)
  {
    ...
    case 100: case 68:           // ------ d - Enter sensor depths ---------------------------------------------
      ...
      Serial.println();
      if(userIn){ EEPROM.put(EEPROM_DEPTHS, depths);} else {};

      delay(500);
      menu();
      break;

    case 'c': case 'C':           // ------ c - Set clock ---------------------------------------------
...
//      delay(500);
    ...
    case 112: case 80:          // ------ p - Print node data to screen ---------------------------------------------
      ...
      Serial.println(F("  t --> Print latest data"));
      Serial.print(F("Enter choice: "));
      charinput();
    ...
    case 101: case 69:          // ------ e - Erase Flash ---------------------------------------------
      ...
      if (toDo == 1) {
        ...
        Serial.println(" done.");
        ft.logSetup(logsBeginAddr, logsEndAddr, LOG_minSize, LOG_maxSize, true);
        EEPROM.update(EEPROM_LASTPRINT,0);
      }
...
//        Serial.print("allSensors[r].depth: ");
//        for(byte j = 0; j < 6; j++){
